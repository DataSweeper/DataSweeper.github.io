...
export default Ember.Component.extend({
  ...
  validGroupPath: Ember.computed('optionGroupPath', {
    get: function get() {
      ...
      if (this.get('debug')) {
        Ember.Logger.log("%c%s#validGroupPath searching for `%s` in Object: %O", "color: purple", // http://www.w3schools.com/html/html_colornames.asp
      ...
      if (typeof lastObject.get(this.get('optionGroupPath')) !== undefined) {
        ...
        Ember.Logger.log("%c%s#validGroupPath Invalid optionGroupPath `%s` in Object: %O.", "color: red", // http://www.w3schools.com/html/html_colornames.asp
  ...
  options: Ember.computed('content', 'optionValuePath', 'optionLabelPath', 'optionGroupPath', {
    get: function get() {
      ...
      if (this.get('validGroupPath')) {
        ...
        if (this.get('debug')) {
          Ember.Logger.log("%c%s#options creating groups: %O from %O", "color: purple", // http://www.w3schools.com/html/html_colornames.asp
      ...
      if (this.get('debug')) {
        Ember.Logger.log("%c%s#options: %O, value: %O", "color: purple", // http://www.w3schools.com/html/html_colornames.asp
  ...
  _updateChosen: function _updateChosen() {
    ...
    Ember.run.next(this, function () {
      if (_this2.get('debug')) {
        Ember.Logger.log("%c%s#_updateChosen HIT...", "color: purple", // http://www.w3schools.com/html/html_colornames.asp
        _this2.toString());
      }
    ...
    if (!this.get('content')) {
      ...
      if (foundSelectedOption.length === 0) {
        foundSelectedOption = this.$('option').filter(function (_i, $el) {
          return Ember.$($el).html() === _this3.get('value');
        });
      }

      if (foundSelectedOption.length === 0) {
        // If still not found, log notification.
        Ember.Logger.log("%c%s#_setup unable to select option with a value or text of `%s`.", "color: red", // http://www.w3schools.com/html/html_colornames.asp
  ...
  actions: {
    selectValue: function selectValue() {
      ...
      if (this.get('debug')) {
        Ember.Logger.log("%c%s#selectValue: %O", "color: purple", // http://www.w3schools.com/html/html_colornames.asp
        this.toString(), value);
      }
